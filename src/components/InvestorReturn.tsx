...
export default function InvestorReturn({ projectId, editable = false }: { projectId: string; editable?: boolean }) {
  ...
  const items = [
    { k: 'Investor Equity', v: money(r.equity), sub: `project needs ~${money(r.projectEquity)}`, col: '#C4973A' },
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10, flexWrap: 'wrap' }}>
        <p style={{ fontSize: 7, letterSpacing: '0.24em', textTransform: 'uppercase', color: '#888', margin: 0, fontWeight: 700 }}>Investor Return — equity IRR</p>
        {editable ? (
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 10, color: '#888' }}>
            Equity cheque
            <span style={{ color: '#AAA' }}>$</span>
            <input type="number" value={equity || ''} step={1_000_000}
              onChange={e => saveEquity(Math.max(0, parseFloat(e.target.value) || 0))}
              style={{ width: 130, textAlign: 'right', fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: 13, color: '#1A1A1A', background: 'transparent', border: 'none', borderBottom: '1px solid #C4973A', padding: '2px 0', outline: 'none' }} />
          </label>
        ) : (
          <span style={{ fontSize: 9, color: '#999' }}>on a {money(r.equity)} equity cheque</span>
        )}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
        {items.map(it => (
          <div key={it.k} style={{ border: '1px solid #E4E1DC', background: '#FFFFFF', padding: '14px 16px' }}>
            <p style={{ fontSize: 7.5, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#888', margin: '0 0 6px' }}>{it.k}</p>
            <p style={{ fontFamily: 'var(--font-mono)', fontWeight: 900, fontSize: 20, color: it.col, margin: 0 }}>{it.v}</p>
            <p style={{ fontSize: 8, color: '#999', margin: '4px 0 0', letterSpacing: '0.04em' }}>{it.sub}</p>
          </div>
        ))}
      </div>
    </div>
