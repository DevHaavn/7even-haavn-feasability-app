    ...
    <div className="flex flex-col">
      ...
      <div className="w-full">
        ...
        <div className="flex mb-4" style={{ display: 'inline-flex', border: '1px solid #D0CEC9' }}>
          {(['management', 'lease'] as const).map((type, i) => (
            <button key={type} onClick={() => setOperatorType(type)}
              ...
              className={`px-4 py-2 text-[10px] tracking-widest uppercase cursor-pointer transition-colors ${operatorType === type ? 'bg-[#1A1A1A] text-white font-semibold' : 'bg-white text-[#888] hover:text-[#1A1A1A]'}`}>
              {type === 'management' ? 'Management Agreement' : 'Operator Lease'}
            </button>
          ))}
        </div>
        {operatorType === 'lease' && (
          <div className="text-[#555] text-xs border border-[#D0D8E8] bg-[#F5F7FC] p-3 mb-4">
        ...
        {(data.buildRateOverride !== undefined || data.constructionFinancePct !== undefined) && (
          <div className="border border-[#C4973A] bg-[#FDFAF5] p-4 mb-4">
            <h3 className="text-[9px] tracking-[0.2em] uppercase text-[#C4973A] mb-3">Construction Cost Override</h3>
            ...
            <div className="mt-2 text-[10px] text-[#888]">Construction cost: <span className="font-mono text-[#1A1A1A]">${(costStack.construction / 1_000_000).toFixed(2)}M</span> · Finance: <span className="font-mono text-[#1A1A1A]">${(costStack.finance / 1_000_000).toFixed(2)}M</span></div>
          </div>
        )}
        ...
        {(data.holdDebtLvr !== undefined || data.holdDebtRate !== undefined) && (
          <div className="border border-[#E8E5E0] bg-white p-4">
            <h3 className="text-[9px] tracking-[0.2em] uppercase text-[#888] mb-3">Hold / Stabilisation Debt</h3>
